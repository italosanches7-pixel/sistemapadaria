...
export type EstadoProduto = { erro?: string; ok?: boolean };
...
export async function cadastrarProduto(_estado: EstadoProduto, formData: FormData): Promise<EstadoProduto> {
  ...
  if (!categoria) return { erro: "Selecione uma categoria." };

  try {
    await prisma.produto.create({ data: { nome, preco, categoria } });
  } catch {
    return { erro: "Não foi possível salvar o produto. Tente novamente." };
  }
...
export async function editarProduto(
  produtoId: string,
  dados: { nome: string; preco: number; categoria: string }
  ...
  if (!categoria) return { sucesso: false, erro: "Selecione uma categoria." };
  ...
  await prisma.produto.update({
    where: { id: produtoId },
    data: { nome, preco: dados.preco, categoria },
  });
